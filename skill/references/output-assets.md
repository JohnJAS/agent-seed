...
The bundled `git-code-tracker` entry is pinned to `v1.0.3` / `66c048422dee2e433583cb97bb3a71efd0fcfbef`. The package root is `packages/git-code-tracker`; the actual platform skills are `.opencode/skills/ai-code-tracker`, `.claude/skills/ai-code-tracker`, and `.cac/skills/ai-code-tracker` inside that package. Prefer its README and `node <package-dir>/install-to-project.js <target-project> --platform <detected-platform>` over duplicating manual install details. Select the detected or requested platform (`opencode`, `claude`, or `codeagent-cli`); use `--platform all` only when the owner explicitly wants all supported agent integrations. Its installer is project-local and may write the selected platform skill directory, selected platform commands/plugins/hooks/settings, `.ai-tracking`, `.gitignore`, and `AGENTS.md`.
